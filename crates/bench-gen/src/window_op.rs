...
//!
//! The task: apply an in-place operation to each consecutive, non-overlapping
//! window of width `w` in a `&mut [i64]`, leaving the trailing remainder
//! untouched, and return the number of full windows processed. Solution-first:
//! the seed picks the *operation* (Reverse / RotateLeft(k) / SwapEnds) and the
//! function name, from which the reference, the example outputs, the differential
//! oracle and the skeleton are all derived — so seeds vary the actual logic a
//! model must produce, not just identifiers.

use crate::{mint_canary, GeneratedTask, Generator, Rng};
use std::collections::BTreeMap;
use std::path::PathBuf;

/// The per-seed structural choice.
...
/// Native reference: apply the op to `v` in place and return the full-window
/// count. Used to compute example outputs; the emitted `reference` source must
/// agree (checked by the reference-passes-its-own-oracle gate).
fn apply(op: &Op, v: &mut [i64], w: usize) -> usize {
    if w == 0 {
        return 0;
    }
    let mut count = 0;
...
fn reference_src(spec: &Spec) -> String {
    format!(
        "pub fn {name}(v: &mut [i64], w: usize) -> usize {{\n\
         \x20   if w == 0 {{ return 0; }}\n\
         \x20   let mut count = 0;\n\
         \x20   for chunk in v.chunks_exact_mut(w) {{\n\
         \x20       {body}\n\
         \x20       count += 1;\n\
         \x20   }}\n\
         \x20   count\n\
...
/// Returns rendered prose and the raw cases.
fn worked_examples(spec: &Spec) -> (String, Vec<ExampleCase>) {
    ...
    for (input, w) in inputs {
        ...
        let count = apply(&spec.op, &mut out, *w);
...
fn prompt(spec: &Spec, canary: &str) -> String {
    ...
    format!(
        "Implement the function `{name}` in `src/lib.rs`.\n\
         \n\
         For each consecutive, non-overlapping window of width `w` in `v`, {prose}. \
         Any trailing elements that do not fill a full window are left untouched. \
         Return the number of full windows processed.\n\
         \n\
         Constraints:\n\
...
fn behavior_test_src(spec: &Spec) -> String {
    ...
    // Edge cases every instance shares.
    body.push_str(&format!(
        "#[test]\nfn zero_width_noop() {{\n\
...
fn differential_test_src(spec: &Spec) -> String {
    // The embedded reference is renamed so it cannot clash with the candidate.
    let reference =
...
impl Generator for WindowOpFamily {
    ...
    fn generate(&self, seed: u64) -> GeneratedTask {
        ...
        GeneratedTask {
            ...
            max_unsafe: 0,
            // borrow-lifetimes: constraint-dominant (docs/04).
...
mod tests {
    ...
    #[test]
    fn seeds_vary_the_operation_not_just_names() {
        // Across many seeds we should see all three ops, i.e. genuine
        // structural variance, not cosmetic renaming.
        let mut ops = std::collections::HashSet::new();
        for seed in 0..60u64 {
            ops.insert(format!("{:?}", sample(seed).op));
        }
        assert!(ops.len() >= 3, "expected structural variety, got {ops:?}");
    }

    #[test]
    fn reference_agrees_with_native_apply() {
        // The emitted reference source and the native `apply` must describe the
        // same function — this is what makes example outputs correct.
        for seed in [1u64, 2, 3, 7, 99] {
            let spec = sample(seed);
            let (_, cases) = worked_examples(&spec);
            for (input, w, expected, count) in cases {
                let mut v = input.clone();
                let c = apply(&spec.op, &mut v, w);
